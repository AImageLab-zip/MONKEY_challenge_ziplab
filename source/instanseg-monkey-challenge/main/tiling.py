...
def is_overlapping(new_pos, existing_positions, tile_size):
    ...
    for existing_x, existing_y in existing_positions:
        if (
            new_x < existing_x + tile_size
            and new_x + tile_size > existing_x
            and new_y < existing_y + tile_size
            and new_y + tile_size > existing_y
        ):
            return True
    return False
...
def find_non_empty_positions(mask, tile_size, downsample_factor_mask, threshold=0.5):
    ...
    for y in range(0, mask_height - scaled_tile_size, scaled_tile_size):
        for x in range(0, mask_width - scaled_tile_size, scaled_tile_size):
            ...
            if (
                mask[y : y + scaled_tile_size, x : x + scaled_tile_size].mean()
                > threshold
            ):
...
def generate_random_tiles_from_valid_positions(
    valid_positions, num_images, tile_size, downsample_factor_mask
):
    """
    ...
    for pos in valid_positions:
        ...
        scaled_pos = (
            int(new_x * downsample_factor_mask),
            int(new_y * downsample_factor_mask),
        )
...
def get_tiles(slide, positions, tile_size=512, level=None):
    ...
    for posx, posy in positions:
        img = slide.read_region(
            (posx, posy), level, (tile_size, tile_size), padding=False, as_array=True
        )
        tiles.append(img)
    return tiles
...
def get_random_non_empty_tiles(slide, slide2=None, num_images=100, tile_size=512):
    ...
    valid_positions = find_non_empty_positions(
        binary_image,
        tile_size=tile_size,
        downsample_factor_mask=downsample_factor_thumbnail,
    )
    positions = generate_random_tiles_from_valid_positions(
        valid_positions,
...
def get_random_non_empty_tiles_with_pos(
    slide, slide2=None, num_images=100, tile_size=512
):
    x_max, y_max = slide.dimensions
    binary_image, downsample_factor_thumbnail = threshold_thumbnail(slide)
    valid_positions = find_non_empty_positions(

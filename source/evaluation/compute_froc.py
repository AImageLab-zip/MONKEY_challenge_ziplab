# Code taken from: https://monai-dev.readthedocs.io/en/latest/_modules/monai/metrics/froc.html
...
def compute_fp_tp_probs_nd(
    ...
    if not (len(probs) == len(coords)):
        raise ValueError(
            f"the length of probs {probs.shape}, should be the same as of coords {coords.shape}."
        )
...
def compute_froc_curve_data(
    fp_probs: np.ndarray | torch.Tensor,
...
def compute_froc_score(
    ...
    interp_sens = np.interp(
        eval_thresholds, fps_per_image[::-1], total_sensitivity[::-1]
    )

...
def plot_overall_froc(
    input_path, output_path, plot_per_file, filename="froc_curves_aggregated.png"
):
    ...
    if plot_per_file:
        for file_id, file_metrics in metric_dict["per_slide"].items():
            ...
            # Save the figure
            # plt.show()
            per_wsi_plots_dir = os.path.join(output_path, "froc_curves_per_wsi")
            os.makedirs(per_wsi_plots_dir, exist_ok=True)
            plt.savefig(
                os.path.join(
                    output_path, per_wsi_plots_dir, f"froc_curves_{file_id}.png"
                )
            )
...
if __name__ == "__main__":
    # EVAL_THRESHOLDS = (10, 20, 50, 100, 200, 300)
    input_path = "/work/grana_urologia/MONKEY_challenge/outputs/cellvit_baseline/scores/metrics.json"
    output_path = (

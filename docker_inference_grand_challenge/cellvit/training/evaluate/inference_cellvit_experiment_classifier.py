...
class CellViTClassifierInferenceExperiment(ABC):
    ...
    def __init__(
        ...
        self.test_result_dir = self._create_inference_directory(
            comment
        )  # creates a test_results directory in the logdir
        self._instantiate_logger()  # creates a logger for the inference
        self.cellvit_model, self.cellvit_run_conf = self._load_cellvit_model(
            checkpoint_path=self.cellvit_path  # loads the cellvit model backbone
        )
        self.model, self.run_conf = self._load_model(
            checkpoint_path=self.model_path
        )  # loads the classifier MLP model
        self.num_classes = self.run_conf[
            "data"
        ][
            "num_classes"
        ]  # number of classes in the dataset (taken from the classifier model run configuration)
        self.inference_transforms = self._load_inference_transforms(
            ...
            transform_settings=self.run_conf.get(
                "transformations", None
            ),  # loads the transformations for the inference if specified in the run configuration,
        )
    ...
    def _load_model(self, checkpoint_path: Union[Path, str]) -> Tuple[nn.Module, dict]:
        ...
        # added log of run configuration to debug
        self.logger.info(f"Run Configuration for the MLP model\n:{run_conf}\n")
    ...
    def _get_classifier_result(
        ...
        # )
        if classifier_output["predictions"]:
            classifier_output["predictions"] = (
                torch.cat(classifier_output["predictions"], dim=0).detach().cpu()
            )
        else:
            classifier_output["predictions"] = torch.empty(0)

        # Do the same for probabilities and ground truth if needed:
        if classifier_output["probabilities"]:
            classifier_output["probabilities"] = (
                torch.cat(classifier_output["probabilities"], dim=0).detach().cpu()
            )
        else:
            classifier_output["probabilities"] = torch.empty(0)

        if classifier_output["gt"]:
            classifier_output["gt"] = (
                torch.cat(classifier_output["gt"], dim=0).detach().cpu()
            )
        else:
            classifier_output["gt"] = torch.empty(0)
